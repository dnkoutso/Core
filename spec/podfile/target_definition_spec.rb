...
module Pod
  describe Podfile::TargetDefinition do
    ...
    describe 'Attributes accessors' do
      ...
      it 'raises if script phase includes an invalid execution placement key' do
        e = lambda { @parent.store_script_phase(:name => 'PhaseName', :script => 'echo "Hello World"', :execution_placement => :unknown) }.should.raise Podfile::StandardError
        e.message.should == 'Invalid execution placement value `unknown` in shell script `PhaseName` within `MyApp` target.}`. ' \
      ...
      it 'stores a script phase if requirements are provided' do
        ...
        @parent.script_phases.should == [
          { :name => 'PhaseName', :script => 'echo "Hello World"', :execution_placement => :any },
        ]
      end
      ...
      it 'stores a script phase with requirements and optional keys' do
        ...
        @parent.script_phases.should == [
          { :name => 'PhaseName', :script => 'echo "Hello World"', :shell_path => '/usr/bin/ruby', :execution_placement => :any },
        ]
      end

      it 'stores a script phase with a specified execution placement value' do
        @parent.store_script_phase(:name => 'PhaseName', :script => 'echo "Hello World"', :shell_path => '/usr/bin/ruby', :execution_placement => :before_compile)
        @parent.script_phases.should == [
          { :name => 'PhaseName', :script => 'echo "Hello World"', :shell_path => '/usr/bin/ruby', :execution_placement => :before_compile },
        ]
      end
