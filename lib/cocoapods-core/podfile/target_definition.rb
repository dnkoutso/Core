module Pod
  class Podfile
    ...
    class TargetDefinition
      ...
      #         `:shell_path`, `:input_files`, `:output_files`, `:show_env_vars_in_log` and `:execution_placement`.
      #
      # @return [void]
      #
      def store_script_phase(options)
        ...
        options[:execution_placement] = :any unless options.key?(:execution_placement)
        unless Specification::EXECUTION_PLACEMENT_KEYS.include?(options[:execution_placement])
          raise StandardError, "Invalid execution placement value `#{options[:execution_placement]}` in shell script `#{options[:name]}` within `#{name}` target.}`. " \
